...
Fully automated: submit domain, wait for mockups, download images.
Auto-detects Microsoft Edge or Chrome.

Usage:
  python goody_auto.py --domain starbucks.com
  python goody_auto.py --domain starbucks.com --output "path/to/2. Swag/"
...
def run(domain, output_dir=None):
    ...
    print(f"Auto-scraping swag for {company} ({domain})")
    ...
    try:
        ...
        time.sleep(2)
        print(f"[OK] Goody loaded")

        # Submit domain
        inputs = driver.find_elements(By.CSS_SELECTOR, "input[type='text']")
        field = next((i for i in inputs if i.is_displayed()), None)
        if not field:
            print("[FAIL] Domain input field not found")
            return False
        field.click()
        time.sleep(0.5)
        field.clear()
        ...
        # Check for "no images" dialog
        no_imgs = driver.find_elements(By.XPATH, "//*[contains(text(),'No images found')]")
        if any(e.is_displayed() for e in no_imgs):
            print("[FAIL] Goody has no images for this domain.")
            print("       Use goody_manual.py to upload a logo instead.")
            return False
        ...
        waited = 0
        while waited < 30:
            if len(driver.find_elements(By.CSS_SELECTOR, MOCKUP_SELECTOR)) >= MAX_IMAGES:
                break
            time.sleep(2)
            waited += 2
        ...
        if not products:
            print("[FAIL] No product mockups found")
            return False

        print(f"[OK] {len(products)} products found — downloading...\n")
...
def main():
    p = argparse.ArgumentParser(description="Auto-download branded swag from ongoody.com")
    p.add_argument("--domain", "-d", help="Company domain (e.g. starbucks.com)")
    p.add_argument("--output", "-o", help="Output folder")
    args = p.parse_args()

    if not args.domain:
        args.domain = input("Enter company domain (e.g. starbucks.com): ").strip()
        if not args.domain or not re.match(r'^[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$', args.domain):
            print("[FAIL] Invalid domain")
            sys.exit(1)

    sys.exit(0 if run(args.domain, args.output) else 1)
