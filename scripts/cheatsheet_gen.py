...
def _generate_meeting_intelligence(company: str, research: dict, model_values: dict,
    ...
    ebitda_total = sum(v for k, v in model_values.items()
                       if k.startswith("ebitda__") and isinstance(v, (int, float)) and v > 0)
    rops_list    = [v for k, v in model_values.items()
                    if k.startswith("rops__") and isinstance(v, (int, float)) and v > 0]
    avg_rops     = sum(rops_list) / len(rops_list) if rops_list else None
...
.data-table tr:nth-child(even) td {{ background: {BG}; }}
...
.bench-table tr:nth-child(even) td {{ background: {BG}; }}
...
.tech-table tr:nth-child(even) td {{ background: {BG}; }}
...
.sum-table tr:nth-child(even) td {{ background: {BG}; }}
